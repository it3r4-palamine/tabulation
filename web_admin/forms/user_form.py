...
class AdminUserForm(UserCreationForm):
    ...
    def __init__(self, *args, **kargs):
        super(AdminUserForm, self).__init__(*args, **kargs)
    ...
    def clean(self):
        ...
        if instance.exists():
            ...
            if instance.pk != self.instance.pk and raw_data["username"]:
                raise ValueError("Username already take")
...
class CustomUserChangeForm(forms.ModelForm):
    class Meta:
        model = User
        fields = (
        "email", "fullname", "is_admin", "is_active", "user_type", "is_edit", "company", "username", "first_name",
        "last_name", "nick_name", "address", "gender", "nationality", "date_of_birth", "contact_number", "fathers_name",
        "mothers_name", "fathers_contact_no", "mothers_contact_no", "grade_level", "school", "description", "rfid")
...
class User_credit_form(forms.ModelForm):
    class Meta:
        model = UserCredit
        fields = (
        'user', 'enrollment_id', 'enrollment_code', 'session_credits', 'session_end_date', 'program_id', 'program_name',
        'session_start_date')
